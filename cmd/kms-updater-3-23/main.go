...
func main() {
	...
	args := os.Args[1:]
	if len(args) < 1 {
		fmt.Println("Please provide a command: create-account, run-kms-update, get-pub-key")
		return
	}
	...
	if args[0] == "create-account" {
		...
		runKmsUpdate()
	} else if args[0] == "get-pub-key" {
		awsClient := createKMSClient(context.Background())
		// awsClient.GetPublicKey(context.Background(), &awskms.GetPublicKeyInput{})


		// im going to need to create a key struct using one of these?
		// type Key struct {
		...
		// or KeyFromResourceARN?

		// what do i have in the db?
		...
		// so i THINK that this means that Storable.Value is the encrypted byte representation of the ARN?
		// ug, getting this decrypted is gonna be annoying...
		// going to try to upgrade the whole flow-wallet-api instead ?
		// oooor, im still not sure thats the best option.. because it seems like a lot of what flow-wallet-api 
		// was necessary for is now in the flow-go-sdk... 
		// but that is not how our workflow is set up.
		// so it probably make sense to upgrade flow-wallet-api first to use the newer flow-go-sdk
		// and then maybe rewrite flow wallet api to be more simple?
		// 

		fmt.Printf("Get public key... %#v \n", awsClient)
	} else {
		fmt.Println("Invalid command. Please provide a valid command: create-account, run-kms-update, get-pub-key")
	}
}

// this is for testing purposes so we can create a new account in the db
func createAccount() {
	...
	fmt.Println("my test private key: ", myPrivateKey)
...
func runKmsUpdate() {
	...
	serviceAcctAddr, _, _ := ServiceAccount(flowClient)
	...
	// the flow-wallet-api signing logic is too old to work with the new - so instead of sending the tx to the flow-wallet-api to sign it
	// let's just sign it here, using the example fro flow-go-sdk

	_, kErr := templates.AccountKeyToCadenceCryptoKey(acctKey)
	if kErr != nil {
		fmt.Println("Error converting account key to cadence crypto key", kErr)
	}

	// accountASigner := is a crypto.Signer
...
func awsKmsSigner(ctx context.Context) {

}

func createKMSClient(ctx context.Context) *awskms.Client {
	awsConfig, err := c.LoadDefaultConfig(ctx)
	if err != nil {
		panic("aws configuration error, " + err.Error())
	}

	awsClient := awskms.NewClient(awsConfig)
